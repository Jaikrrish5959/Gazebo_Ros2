...
def generate_launch_description():
    ...
    world_gen_script = os.path.join(pkg_warehouse, '..', '..', 'lib',
                                    'forklift_warehouse',
                                    'random_world_generator.py')
    if not os.path.exists(world_gen_script):
        world_gen_script = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            'src', 'random_world_generator.py')

    dynamic_world_file = '/tmp/warehouse_dynamic.sdf'
    subprocess.run(
        ['python3', world_gen_script, '--output', dynamic_world_file],
        check=True,
    )
    world_file = dynamic_world_file
    ...
    adaptive_box_transport_node = Node(
        package='forklift_warehouse',
            ...
            on_exit=[
                TimerAction(
                    ...
                    actions=[
                        ...
                        adaptive_box_transport_node,
                        ontology_reasoner_node,
                    ],
                )
            ],
        )
