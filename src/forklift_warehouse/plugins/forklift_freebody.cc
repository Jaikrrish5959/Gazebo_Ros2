...
class ForkLiftFreeBodyPlugin : public gz::sim::System,
...
private:
  ...
  gz::sim::Entity shift_left_joint_;
  gz::sim::Entity shift_right_joint_;
...
public:
  ForkLiftFreeBodyPlugin() = default;
  ~ForkLiftFreeBodyPlugin() override = default;
  ...
  void Configure(const gz::sim::Entity &_entity,
    ...
    this->ros_node_ = rclcpp::Node::make_shared(this->model_name_ +
                                                "_forklift_freebody_plugin");
    ...
    this->joint_state_sub_ =
        this->ros_node_->create_subscription<sensor_msgs::msg::JointState>(
            "/joint_states", 10,
            std::bind(&ForkLiftFreeBodyPlugin::OnJointState, this,
                      std::placeholders::_1));
    ...
    for (const auto &joint : joints) {
      ...
      if (joint_name_comp) {
        ...
        // Look for "shift_left" in joint name
        if (joint_name.find("shift_left") != std::string::npos) {
          this->shift_left_joint_ = joint;
          RCLCPP_INFO(this->ros_node_->get_logger(),
                      "Found shift_left joint: %s", joint_name.c_str());
        }
        // Look for "shift_right" in joint name
        else if (joint_name.find("shift_right") != std::string::npos) {
          this->shift_right_joint_ = joint;
          RCLCPP_INFO(this->ros_node_->get_logger(),
                      "Found shift_right joint: %s", joint_name.c_str());
        }
  ...
  void PreUpdate(const gz::sim::UpdateInfo & /*_info*/,
    ...
    for (const auto &[joint_name, position] : this->joint_commands_) {
      if (joint_name == "shift") {
        // Apply to both left and right shift joints
        if (this->shift_left_joint_ != gz::sim::kNullEntity) {
          auto pos_reset_comp =
              _ecm.Component<gz::sim::components::JointPositionReset>(
                  this->shift_left_joint_);
          if (!pos_reset_comp) {
        ...
        if (this->shift_right_joint_ != gz::sim::kNullEntity) {
          auto pos_reset_comp =
              _ecm.Component<gz::sim::components::JointPositionReset>(
                  this->shift_right_joint_);
          if (!pos_reset_comp) {
            _ecm.CreateComponent(
                this->shift_right_joint_,
                gz::sim::components::JointPositionReset({position}));
...
// Register the plugin
GZ_ADD_PLUGIN(forklift_gazebo::ForkLiftFreeBodyPlugin, gz::sim::System,
              forklift_gazebo::ForkLiftFreeBodyPlugin::ISystemConfigure,
              forklift_gazebo::ForkLiftFreeBodyPlugin::ISystemPreUpdate)
