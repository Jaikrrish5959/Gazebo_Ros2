...
class ForkLiftWorld : public gz::sim::System, public gz::sim::ISystemConfigure {
private:
  rclcpp::Node::SharedPtr ros_node_;

public:
  ForkLiftWorld() = default;
  ~ForkLiftWorld() override = default;
  ...
  void Configure(const gz::sim::Entity &_entity,
    ...
    this->ros_node_ = rclcpp::Node::make_shared("forklift_world_plugin");

    RCLCPP_INFO(this->ros_node_->get_logger(), "ForkLiftWorld plugin loaded");

    // Create entity creator
    gz::sim::SdfEntityCreator creator(_ecm, _eventMgr);

    // Load and spawn the forklift model
    const int NUM_BOTS = 1;
    RCLCPP_INFO(this->ros_node_->get_logger(), "Total bots: %d", NUM_BOTS);

    // Spawn forklift at origin
    this->AddModel(creator, "fork_lift_shift", "forklift", 0.0, 0.0, 0.0);
  }
...
// Register the plugin
GZ_ADD_PLUGIN(forklift_gazebo::ForkLiftWorld, gz::sim::System,
              forklift_gazebo::ForkLiftWorld::ISystemConfigure)
